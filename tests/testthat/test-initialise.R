...
test_that("initialise environment", {
  withr::with_envvar(empty_env, {
    ...
    expect_equivalent(Sys.getenv("MODULEPATH", unset = NA),
...
test_that("version not overwritten", {
  ...
  withr::with_envvar(version_set, {
    moduleInit(modulesHome = modulesHome)
    expect_equivalent(Sys.getenv("MODULE_VERSION", unset = NA),
                      as.character(version_set[["MODULE_VERSION"]]))
    expect_equivalent(Sys.getenv("MODULE_VERSION_STACK", unset = NA),
                      as.character(version_set[["MODULE_VERSION"]]))
  })
})
...
test_that("loaded modules not overwritten", {
  ...
  modpath_set[["MODULEPATH"]] <-
    "/usr/share/Modules/modulefiles:/opt/share/modulefiles"
...
test_that("checking modulecmd works", {
  withr::with_envvar(empty_env, {
    expect_error(
      moduleInit(modulesHome = "/opt/usr/local/unlikely/1.2.3"),
      paste(
        "/opt/usr/local/unlikely/1.2.3/bin/modulecmd missing!",
