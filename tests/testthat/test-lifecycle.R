...
test_that("package lifecycle", {
  ...
  withr::with_envvar(empty_env, {
    expect_true(is.na(version_set <-
                        Sys.getenv("MODULE_VERSION", unset = NA)))
    expect_true(is.na(
      version_set <- Sys.getenv("MODULE_VERSION_STACK", unset = NA)
    ))
    ...
    withr::with_envvar(c(
      ...
      expect_equivalent(Sys.getenv("MODULE_VERSION", unset = NA), "4.4", label = "set")
